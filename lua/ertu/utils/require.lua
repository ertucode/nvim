...
local function resolve_relative(path, __dirname)
	if path:match("^%.%./") then
		...
		local segments = {}
		for str in __dirname:gmatch("([^/]+)") do
	...
	if not path:match("%.") and path:match("/") then
		if path:match(__dirname) then
			return path
		end
		if path:match(__dirname:gsub("%./", "")) then
			return path
		end
	end
...
---@param path any
---@return string
local function to_require_arg(path)
...
---@param path any
---@return unknown
function M.relative_require(path)
