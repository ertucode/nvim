...
---@param opts table
---@return vim.quickfix.entry[]
local function filter_lines_with_import(items, opts)
	return vim.tbl_filter(function(item)
		print(item)
		return true
	end, items)
...
local function list_or_jump(action, title, funname, params, opts)
	...
	vim.lsp.buf_request_all(opts.bufnr, action, params, function(results_per_client)
		...
		items = filter_lines_with_import(items, opts)
		...
		for client_id, client_res in pairs(results) do
			...
			elseif client_res.result ~= nil then
				if vim.fn.has("nvim-0.11") == 1 then
					...
					vim.list_extend(
						locations,
						vim.lsp.util.symbols_to_items(client_res.result, bufnr, client.offset_encoding)
					)
				else
